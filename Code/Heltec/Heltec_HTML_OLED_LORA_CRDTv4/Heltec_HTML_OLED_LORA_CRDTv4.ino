...
WebServer webServer(80);        // HTTP-Server on port 80
char ssid[32];
...
void sendIDList() {
  ...
  String out = "IDLIST|";
  // First own entry
  auto itSelf = idlist.find(nodeID);
  if (itSelf != idlist.end()) {
    const IDElement& me = itSelf->second;
    out += String(me.id) + "_" + nodeID + "_" + String(me.numberOfMessages) + "|";
  }
  // Then the rest
  for (auto &kv : idlist) {
...
int getIndexAfterId(const String& targetId) {
  for (int i = 0; i < crdtList.size(); ++i) {
    if (crdtList[i].id == targetId) {
      if (i + 1 < crdtList.size())  
        return i + 1;
      else
        return -1;
    }
...
void createRemoteList(String& list, std::map<String, IDElement, MacLess>& remotelist, String& senderMAC) {
...
void handleIDList(String list) {
  ...
  // Parse, create the remote list
  createRemoteList(list, remotelist, senderMAC);
  
  // If the element of the peer does not yet exists it gets created
  if (idlist.find(senderMAC) == idlist.end()) {
    idlist[senderMAC] = { 0, 0 };
  }
  // Information over the peer in own list
  IDElement &peerInfo = idlist[senderMAC];
  // Information over ourself in own list
  IDElement &localInfo = idlist[nodeID];

  // If no entry for us, start sending all msg (but chunk-wise)
  if (remotelist.find(nodeID) == remotelist.end()) {
    Serial.println("Sending chunk-wise bc sender doesnt know me:");
    sendMessageChunk(peerInfo);
    return;
  }

  // Information over ourself from peers list
  IDElement &usInPeer = remotelist[nodeID];

  // If remote id is lower than our id then send what comes after chunk-wise
  if (usInPeer.id < localInfo.id) {
    Serial.println("Sending chunk-wise:");
    peerInfo.chunkIndex = getIndexAfterId((String) usInPeer.id);
    sendMessageChunk(peerInfo);

  // If IDs are correct but the number of msg not, there was an unrecognized msg
  // is a huge mess but we should not get here!!!
  } else if (localInfo.id == usInPeer.id && 
      localInfo.numberOfMessages != usInPeer.numberOfMessages) {
    Serial.println("Sending chunk-wise bc error in transit:");
    sendMessageChunk(peerInfo);
  }
}
